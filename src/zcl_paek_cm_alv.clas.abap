class ZCL_PAEK_CM_ALV definition
  public
  create public .

public section.

  interfaces ZIF_PAEK_CM .
  ...
  types:
    BEGIN OF TY_OALV,
            ...
            GRID TYPE REF TO ZCL_PAEK_CM_OALV,
            ...
            HANDLER_EVENTS  TYPE REF TO ZCL_PAEK_CM_ALV_EVENT,
          END OF TY_OALV .
  types:
    BEGIN OF TY_OTREE,
            REPID TYPE SY-REPID,
            ...
            SAVEMODE TYPE CHAR1,
            VARIANT TYPE disvariant,
            HANDLER_EVENTS  TYPE REF TO ZCL_PAEK_CM_TREE_EVENT,
          END OF TY_OTREE .
  types:
  ...
  data MO_ALV_EVENT type ref to ZCL_PAEK_CM_ALV_EVENT .
  data MO_TREE_EVENT type ref to ZCL_PAEK_CM_TREE_EVENT .
  ...
  methods CREATE_ALV_GRID
    ...
    returning
      value(RO_OALV) type ref to ZCL_PAEK_CM_OALV
    raising
      ZCX_PAEK_EXCP .
  methods CREATE_ALV_TREE
    ...
    returning
      value(RO_OTREE) type ref to ZCL_PAEK_CM_OTREE
    raising
      ZCX_PAEK_EXCP .
  ...
  methods SET_EVENT_HANDLER
    importing
      ...
      !IV_ROW type I optional
      !IV_COLUMN type I optional
      !IO_EVENT type ref to ZCL_PAEK_CM_ALV_EVENT .
  ...
  methods SET_TREE_EVENT_HANDLER
    importing
      ...
      !IV_ROW type I optional
      !IV_COLUMN type I optional
      !IO_EVENT type ref to ZCL_PAEK_CM_TREE_EVENT .
...
CLASS ZCL_PAEK_CM_ALV IMPLEMENTATION.


    method CREATE_ALV_GRID.

      DATA : LO_OALV TYPE REF TO ZCL_PAEK_CM_OALV.
      ...
      IF LO_OALV IS BOUND.
        ...
        MV_COLUMN = IV_COLUMN.
      ELSE.
        ZCX_PAEK_EXCP=>RAISE( iv_message = 'Cannot Create Grid' ).
      ENDIF.
  ...
  method CREATE_ALV_TREE.
      ...
      DATA : LO_OTREE TYPE REF TO ZCL_PAEK_CM_OTREE.
      ...
      IF LO_OTREE IS BOUND.
        ...
        MV_COLUMN = IV_COLUMN.
      ELSE.
        ZCX_PAEK_EXCP=>RAISE( IV_MESSAGE = 'Cannot Create Tree' ).
      ENDIF.
...
*            CONTAINER TYPE REF TO CL_GUI_CONTAINER,
  ...
  METHOD CREATE_SPLIT_CONTAINER.
    ...
    IF IV_ROWS > 3 OR IV_COLUMNS > 3.
      ZCX_PAEK_EXCP=>RAISE( IV_MESSAGE = 'No more than 3 row or column' ).
    ENDIF.
    ...
    ELSE.
      ZCX_PAEK_EXCP=>RAISE( IV_MESSAGE = 'Why you set with so many containers?' ).
    ENDIF.
  ...
  method DISPLAY_ALV.
    ...
    IF SY-SUBRC <> 0.
      ZCX_PAEK_EXCP=>RAISE( iv_message = 'Cannot Fetch Grid in display' ).
    ENDIF.
  ...
  METHOD DISPLAY_TREE.
    ...
    READ TABLE MT_OTREE ASSIGNING FIELD-SYMBOL(<FS_OTREE>)
                                  WITH KEY REPID = IV_REPID
                                           ...
                                           COLUMN = LV_COLUMN.
    IF SY-SUBRC <> 0.
      ZCX_PAEK_EXCP=>RAISE( iv_message = 'Cannot Fetch Tree in display' ).
    ENDIF.
...
*                            ( CL_GUI_ALV_GRID=>MC_FC_LOC_INSERT_ROW    )
  ...
  method ZIF_PAEK_CM~CREATE_DYNAMIC_TABLE.
  ...
  method ZIF_PAEK_CM~SALV_AFTER_SALV_FUNCTION.
    " Use SALV classes for this method.
  endmethod.


  method ZIF_PAEK_CM~SALV_BEFORE_SALV_FUNCTION.
    " Use SALV classes for this method.
  endmethod.


  method ZIF_PAEK_CM~SALV_END_OF_PAGE.
    " Use SALV classes for this method.
  endmethod.


  method ZIF_PAEK_CM~SALV_GET_SELECTED_ROWS.
    " Use SALV classes for this method.
  endmethod.


  method ZIF_PAEK_CM~SALV_ON_DOUBLE_CLICK.
    " Use SALV classes for this method.
  endmethod.


  method ZIF_PAEK_CM~SALV_ON_FUNCTION_CLICK.
    " Use SALV classes for this method.
  endmethod.


  method ZIF_PAEK_CM~SALV_ON_LINK_CLICK.
    " Use SALV classes for this method.
  endmethod.


  method ZIF_PAEK_CM~SALV_TOP_OF_PAGE.
    " Use SALV classes for this method.
  endmethod.
ENDCLASS.
